...
fn test_treasury_withdrawal_exceeding_limit_fails() {
    ...
    // Cannot use the helper because reset_protection_limit after setting 0% would treat 0 as
    // "no override" and fall back to the default 5%.  Call change_protection_limit_percent
    // directly so the 0% override sticks without a subsequent reset.
