...
mod ReplaceCollateralEIC {
    ...
    impl EICInitializable of IEICInitializable<ContractState> {
        fn eic_initialize(ref self: ContractState, eic_init_data: Span<felt252>) {
            ...
            assert!(
                usdc_e_contract_address == self.usdc_e_contract.read().contract_address,
                "Invalid USDC E contract address",
            );
            assert!(
                usdc_contract_address == self.usdc_contract.read().contract_address,
            ...
            let usdc = IERC20Dispatcher { contract_address: usdc_contract_address };
            ...
            assert!(
                ...
                "replace collateral address must be only after 50% already migrated",
            )
