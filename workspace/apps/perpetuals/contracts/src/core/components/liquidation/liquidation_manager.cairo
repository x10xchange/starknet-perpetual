...
pub(crate) mod LiquidationManager {
    ...
    impl LiquidationManagerImpl of ILiquidationManager<ContractState> {
        ...
        fn liquidate(
            ...
            /// Validations:
            self._validate_liquidate(:liquidated_order, :liquidator_order, :actual_liquidator_fee);
    ...
    pub impl InternalFunctions of LiquidationManagerFunctionsTrait {
        fn _validate_liquidated_position(
            ...
            let entry = (@self).assets.asset_config.entry(asset_diff_id).as_ptr();
        ...
        fn _validate_liquidate<T, +ValidateableOrderTrait<T>, +Drop<T>, +Copy<T>>(
            ref self: ContractState,
            liquidated_order: T,
            liquidator_order: T,
            actual_liquidator_fee: u64,
        ...
        fn _execute_liquidate<T, +ValidateableOrderTrait<T>, +Drop<T>, +Copy<T>>(
            ref self: ContractState,
            liquidated_position_id: PositionId,
            ...
            let (
                liquidator_position_diff, optional_liquidator_receive_position_diff,
            ): (PositionDiff, Option<PositionDiff>) =
                if liquidator_order
                .receive_position() != liquidator_order
                .position_id() {
                (
                    PositionDiff {
                        collateral_diff: -liquidated_order.quote_amount().into()
